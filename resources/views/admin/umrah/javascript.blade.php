<script>
    ...
    $(document).ready(function() {
        ...
        // Show Data
        var table = $('.tableUmrah').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('umrah.admin.data') }}",
            'columnDefs': [{
        ...
        // Create Modal
        $('#addUmrah').click(function() {
            $('#frm_umrah').trigger("reset");
            $('#modalUmrah').modal('show');
            ...
            $("#status").empty();
            $("#status").append('<option value="">Choose Payment Type</option>');
            $("#status").append('<option value="Available">Available</option>');
            $("#status").append('<option value="Not Available">Not Available</option>');
        });
        ...
        $('#saveBtn').click(function(e) {
            ...
            var formData = new FormData($('#frm_umrah')[0]);
            ...
            if (idEdit === 0) {
                url = "{{ route('umrah.admin.store') }}"
                type = "POST"
                console.log(formData)
            } else {
                url = "{{ route('umrah.admin.update', ':id') }}";
        ...
        $('body').on('click', '#edit', function() {
            ...
            var url = '{{ route('umrah.admin.edit',':id') }}'
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    base_thumbnail = 'http://localhost:8000/asset_web/paket/umrah/' + encodeURIComponent(res.data.thumbnail) + ''
                    console.log(base_thumbnail)
                    base_banner = 'http://localhost:8000/asset_web/paket/umrah/' + encodeURIComponent(res.data.banner) + ''
                    idEdit = res.data.id;
                    $('#frm_umrah').trigger("reset");
                    $('#modalUmrah').modal('show');
                    $('#nama').val(res.data.nama);
                    var hargaRupiah = formatAngka(res.data.harga);
                    $('#harga').val('Rp ' +hargaRupiah);
                    $('#hotel').val(res.data.hotel);
                    $('#maskapai').val(res.data.maskapai);
                    $('#hotel').val(res.data.hotel);
                    ...
                    $('#preview-thumbnail').attr('src', '{{ url("asset_web/paket/umrah/") }}/' + encodeURIComponent(res.data.thumbnail));
                    $('#preview-banner').attr('src', '{{ url("asset_web/paket/umrah/") }}/' + encodeURIComponent(res.data.banner));
        ...
        $('body').on('click', '#delete', function() {
            var id = $(this).attr('data-id');
            var url = '{{ route('umrah.admin.delete',':id') }}';
            url = url.replace(':id', id);
            Swal.fire({
                    title: 'Anda Yakin ?',
